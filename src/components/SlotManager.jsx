...
function SortableSlot({ slot, index, onUpdate, onDelete, entities }) {
  ...
  const sizeLabel = SLOT_SIZES.find(s => s.id === slot.size)?.label ?? 'Small';
  ...
  return (
    <div ref={setNodeRef} style={style} className={`slot-item${isDragging ? ' dragging' : ''}`}>
      <div className="slot-header">
        <span className="slot-drag" {...attributes} {...listeners} title="Ziehen zum Sortieren">⠿⠿</span>
        ...
        <button className="slot-delete" onClick={() => onDelete(slot.id)} title="Slot löschen">✕</button>
      </div>
        ...
        <div className="slot-body">
          ...
          <div className="form-group">
            ...
            <div className="entity-input-row">
              <input
                value={slot.entityId}
                onChange={e => onUpdate(slot.id, 'entityId', e.target.value)}
          ...
          <div className="form-group">
            ...
            <input
              value={slot.unit}
              onChange={e => onUpdate(slot.id, 'unit', e.target.value)}
              placeholder="°C"
            />
          </div>
...
export default function SlotManager({ slots, onChange, entities }) {
  ...
  const addSlot = () => {
    ...
    onChange([...slots, { id: `${Date.now()}`, title: 'Neuer Slot', unit: '', entityId: '', size: 'small' }]);
  };
  ...
  return (
    <div className="slot-manager">
      <div className="slot-manager-header">
        <div className="section-title" style={{ marginBottom: 0 }}>Slots ({slots.length}/16)</div>
        <button className="add-slot-btn" onClick={addSlot} disabled={slots.length >= 16}>+ Slot hinzufügen</button>
      </div>
      ...
      {slots.length === 0 && (
        <div className="slot-empty">Noch keine Slots – füge deinen ersten hinzu!</div>
      )}
    </div>
  );
}
