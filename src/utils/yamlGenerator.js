...
export function generateYaml(config, slots) {
  const { display, board, title, deviceName, displayName, spiPins, deepSleep, updateInterval, gridCols, batteryEntityId } = config;
  ...
  const sensorSection = sensorLines.length ? `sensor:\n${sensorLines.join('\n')}\n` : '';
...
function buildLambda(config, layout, maxRows, hasBatt) {
  ...
  // Battery reserves space on the right; time shifts left accordingly
  const battReserve = hasBatt ? 56 : 0;
  const timeX = w - pad - 4 - battReserve;
  ...
  if (hasBatt) {
    lines.push(`      `);
    lines.push(`      // ── Batterie-Anzeige ──`);
    lines.push(`      if (!std::isnan(id(battery_level).state)) {`);
    lines.push(`        int batt = (int)id(battery_level).state;`);
    lines.push(`        char batt_buf[8];`);
    lines.push(`        snprintf(batt_buf, sizeof(batt_buf), "%d%%", batt);`);
